...
// $Revision: 8692 $ $Date:: 2018-02-06 #$ $Author: serge $
...
void LogfileTimeWriter::write( const log_levels_log4j level, const char *module_name, const std::string & msg )
{
    ...
    Logfile::OneLiner( logfile_ ).get() << get_current_timestamp( s ) << " " << dummy_logger::to_string( level ) << "|" << module_name << ": " << msg << "\n";
}
...
void LogfileTimeWriter::write( const log_levels_log4j level, const char *module_name, unsigned int inst_id, const std::string & msg )
{
    ...
    Logfile::OneLiner( logfile_ ).get() << get_current_timestamp( s ) << " " << dummy_logger::to_string( level ) << "|" << module_name << "[" << inst_id << "]: " << msg << "\n";
}
...
void LogfileTimeWriter::write( const char *module_name, const std::string & msg )
{
    ...
    Logfile::OneLiner( logfile_ ).get() << get_current_timestamp( s ) << " " << module_name << ": " << msg << "\n";
}
...
void LogfileTimeWriter::write( const char *module_name, unsigned int inst_id, const std::string & msg )
{
    ...
    Logfile::OneLiner( logfile_ ).get() << get_current_timestamp( s ) << " " << module_name << "[" << inst_id << "]: " << msg << "\n";
}
